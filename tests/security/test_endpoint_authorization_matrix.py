...
def test_every_operation_denies_unauthenticated_access(case: OperationCase, runtime_config) -> None:
...
@pytest.mark.runtime
@pytest.mark.security
@pytest.mark.parametrize("case", WRONG_ROLE_CASES, ids=lambda case: case.operation_id)
def test_every_restricted_operation_denies_a_wrong_role(
    case: OperationCase, request: pytest.FixtureRequest
) -> None:
...
def test_every_operation_is_reachable_by_an_allowed_role(
    ...
    _pace_mutation(case, fixture)
    response = client.request(case.method, case.path, **_request_kwargs(case))
    assert response.status_code in case.declared_statuses, (
        case.operation_id,
        response.status_code,
        sorted(case.declared_statuses),
        response.text,
    )
    assert response.status_code not in {401, 403, 429}
