...
The external migration runner installed and verified 46 ordered assets against local Oracle ADB Free in ATP mode. A restart/rerun completed successfully with exact 18-table, 189-column, 17-foreign-key parity and zero invalid objects.
...
| Total | 46 | Complete install manifest |
...
The runner records source checksums in ignored evidence rather than adding an unsupported application table. It skips only unchanged assets when the live schema has the expected 18-table fingerprint; validators always rerun. Package-spec changes force package recompilation. Three idempotent seed files populate all 18 tables, then identity values are synchronized and seed/retry invariants are checked.
